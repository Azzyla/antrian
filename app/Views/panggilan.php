...
<body>
          ...
          <table class="table table-bordered table-striped">
            ...
            <tbody>
              ...
              foreach ($sudahDipanggil as $row):
                if (session('role') === 'cs' && ($row['cs_role'] ?? '') !== session('role')) continue;
              ?>
                <tr>
                  <td><?= $no++ ?></td>
                  <td><?= ucfirst($row['kategori'] ?? '-') ?></td>
  ...
  <script>
    ...
    function panggil(button, jenis = 'panggil') {
      ...
      // Tentukan huruf kategori
      let huruf = 'A'; // mahasiswa
      ...
      const nomorInt = nomor.replace(/^\D+/g, ''); // hapus huruf, ambil angka
    ...
    function playNextAudio() {
      if (audioQueue.length === 0) {
        isPlaying = false;
        if (pendingRedirects.length > 0) window.location.href = pendingRedirects.pop();
        return;
      }
      ...
      // if (pendingRedirects.length > 0) window.location.href = pendingRedirects.pop();
      if (pendingRedirects.length > 0) window.location.href = pendingRedirects.shift();
