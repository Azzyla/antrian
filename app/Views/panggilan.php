...
$csDisplay = ucfirst($csName ?? 'Guest') . ' ' . strtoupper($csRole ?? '');
?>
...
<div class="d-flex">
  <nav class="bg-dark text-white p-3 d-flex flex-column" style="width: 250px; height: 100vh;">
    <div class="text-center mb-4">
      ...
      <div class="bg-secondary rounded p-2 mt-2 text-white fw-bold"><?= $csDisplay ?></div>
    </div>
        ...
        <table class="table table-bordered table-striped">
          ...
          <tbody>
            <?php
              $no = 1;
              $csAktif = false;
              foreach ($sudahDipanggil as $cek) {
                  if ($cek['cs_role'] === session('role') && $cek['status'] === 'dilayani') {
                      $csAktif = true;
                      break;
                  }
              }
...
} elseif (!empty($row['waktu_mulai']) && $row['status'] === 'dilayani') {
    $id = $row['id'];
    $waktuMulaiISO = date('Y-m-d\TH:i:s', strtotime($row['waktu_mulai']));
    echo '<span id="durasi-' . $id . '">0 detik</span>';
    ?>
    <script>
    const mulai<?= $id ?> = new Date("<?= $waktuMulaiISO ?>");
    function updateDurasi<?= $id ?>() {
        const now = new Date();
        const diffMs = now - mulai<?= $id ?>;
        const diffSec = Math.floor(diffMs / 1000);
        const menit = Math.floor(diffSec / 60);
        const detik = diffSec % 60;
        const el = document.getElementById('durasi-<?= $id ?>');
        if (el) el.innerText = `${menit} menit ${detik} detik`;

        // Auto selesai jika durasi sudah lebih dari 5 menit (300000 ms)
        if (diffMs >= 5 * 60 * 1000) {
            window.location.href = "/panggilan/selesaiLayanan/<?= $id ?>";
        }
    }
...
?>
                ...
                <td>
                  <?php if ($row['status'] === 'dipanggil'): ?>
                    ...
                    <?php else: ?>
                      <button class="btn btn-warning btn-sm" disabled>Mulai Layanan</button>
                    <?php endif; ?>
...
function panggil(button, jenis = 'panggil') {
  ...
  const huruf = kategori === 'dosen' ? 'B' : kategori === 'umum' ? 'C' : 'A';
  const role = "<?= session('role') ?>";
  const loket = role.replace('cs', '');
